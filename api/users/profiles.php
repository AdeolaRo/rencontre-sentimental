...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        elseif (isset($_GET['type']) && $_GET['type'] === 'explore') {
            ...
            if ($department) { $countSql .= " AND department = :dept"; $countParams['dept'] = $department; }
            if ($city) { $countSql .= " AND city = :city"; $countParams['city'] = $city; }
    ...
    case 'PUT':
        ...
        $stmt = $db->prepare("UPDATE users SET first_name=?, last_name=?, city=?, department=?, title=?, description=? WHERE id=?");
        $stmt->execute([$data['firstName'], $data['lastName'], $data['city'], $data['department'], $data['title'], $data['description'], $user['id']]);
        ...
        if (isset($data['questionnaire'])) {
            ...
            if ($check->fetch()) {
                $stmtQ = $db->prepare("UPDATE user_questionnaire SET personality=?, preferences=?, style=?, music_tastes=?, passions=?, lifestyle=? WHERE user_id=?");
                $stmtQ->execute([$q['personality'], $q['preferences'], $q['style'], $q['musicTastes'], $q['passions'], $q['lifestyle'], $user['id']]);
            } else {
                $stmtQ = $db->prepare("INSERT INTO user_questionnaire (user_id, personality, preferences, style, music_tastes, passions, lifestyle) VALUES (?,?,?,?,?,?,?)");
                $stmtQ->execute([$user['id'], $q['personality'], $q['preferences'], $q['style'], $q['musicTastes'], $q['passions'], $q['lifestyle']]);
            }
        }
